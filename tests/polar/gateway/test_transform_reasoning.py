...
Each test:
1. Builds a chat completion with `reasoning_content`.
...
def _chat_response(*, with_tool_call: bool = False) -> dict:
    message: dict = {
        "role": "assistant",
        "content": ANSWER_TEXT,
...
def test_openai_chat_preserves_reasoning_content_on_response() -> None:
    ...
    out = t.transform_response(_chat_response(), {"model": "anything"})
...
def test_anthropic_response_emits_thinking_block_before_text() -> None:
    ...
    out = t.transform_response(_chat_response(), {"model": "claude-3"})
    blocks = out["content"]
...
def test_anthropic_response_emits_thinking_with_tool_use() -> None:
    ...
    out = t.transform_response(
        _chat_response(with_tool_call=True), {"model": "claude-3"}
    )
...
def test_gemini_response_emits_thought_part_before_text() -> None:
    t = GoogleTransformer()
    out = t.transform_response(_chat_response(), {})
...
def test_responses_response_emits_reasoning_item_first() -> None:
    ...
    out = t.transform_response(_chat_response(), {"model": "gpt-5.4"})
    items = out["output"]
