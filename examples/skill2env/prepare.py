#!/usr/bin/env python3
"""Prepare Skill2Env Apptainer images and Molt prompt records."""
...
DEFAULT_DATASET = Path("/raid/binfeng/data/s2e/s2ev2_full_8k")
DEFAULT_IMAGES = Path("/raid/binfeng/data/s2e/s2ev2_sif")
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "training.jsonl"
HERMES_INSTALL = (
    'export PATH="$HOME/.local/node/bin:$HOME/.local/bin:$PATH"; '
    'mkdir -p "$HOME/.cache"; '
    'flock "$HOME/.cache/skill2env-hermes.lock" bash -c \''
    'set -e; export PATH="$HOME/.local/node/bin:$HOME/.local/bin:$PATH"; '
    "hermes --version >/dev/null 2>&1 || "
    "(curl -LsSf https://astral.sh/uv/install.sh | sh && "
    "uv tool install --force --managed-python --python 3.12 hermes-agent==0.15.1); "
...
def main() -> int:
    ...
    parser.add_argument("--max-tasks", type=int, default=40, help="Acceptance set size; -1 selects all tasks.")
    ...
    for index, task_file in enumerate(task_files, 1):
        ...
        if args.force or not image.is_file() or image.stat().st_size == 0:
            ...
            source = dockerfile.read_text().rstrip() + "\n" + RUNTIME_LAYER.lstrip()
            ...
            try:
                subprocess.run(
                    [apptainer, "build", "--force", str(temporary), f"docker-daemon://{image_ref}"], check=True
                )
        ...
        if not args.skip_image_check:
            subprocess.run(
                [
                    ...
                    "sh",
                    "-lc",
                    "test -d /workspace && test -x /usr/bin/curl && command -v flock >/dev/null",
                ],
                check=True,
            )
        ...
        spec = TaskSpec.model_validate(
            {
                "runtime": {
                    ...
                    "prepare": [{"type": "exec", "command": HERMES_INSTALL}],
                    "env": {"HOME": "/root"},
                    "network": "host",
                    "allow_internet": True,
                    "workdir": "/workspace",
    ...
    print(f"Wrote {len(rows)} validated Molt records to {args.output}")
    return 0
