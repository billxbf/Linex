...
`polar.rollout` is the **central orchestrator**. A trainer or example script
posts a task here; the rollout server fans it out into one session per sample,
schedules each onto a healthy gateway node, collects the terminal result, and
...
- `server.py`: FastAPI app — task submit/get/status, node register/heartbeat/
  list/drain, the session-result callback, and `/health`.
