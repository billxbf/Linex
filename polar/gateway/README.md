...
2. **transforms** the request to the served model and adds training fields
   (`transform/`),
3. forwards it to the configured vLLM server,
...
- `server.py`: the FastAPI app — the catch-all LLM proxy route, the
  session/admin/health endpoints, and synthetic streaming.
...
- `proxy.py`: `InferenceClient`, the vLLM HTTP client with token-id/logprob
  capture and pause/resume generation gating.
