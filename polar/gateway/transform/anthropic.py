...
# multi-turn traces. Strip the line before forwarding it to inference.
...
class AnthropicStreamState:
    ...
    single upstream chat-completions stream.
    """
...
class AnthropicTransformer(BaseTransformer):
    ...
    def _transform_tools_to_openai(self, tools: list[dict]) -> list[dict]:
        ...
        for tool in tools:
            ...
            # explicit `type` and have no `input_schema`. Drop them rather than
            # forwarding a stub function tool that cannot be dispatched.
