...
nodes (each with its own vLLM endpoint). `TopologyConfig.load()` is the entry
point used by both services.
...
Set `POLAR_GATEWAY_NODE_ID` when the topology has more than one node so each
gateway process starts with the right ports, worker limits, and inference
endpoint.
